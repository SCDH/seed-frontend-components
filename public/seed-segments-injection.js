...
const seedPostMessageOnSegmentEvent = (eventType) => {
    return function (e) {
        ...
        window.parent.postMessage(
            { ...msg, event: eventType, segmentIds: ids },
...
function seedAncestorOrSelfIds(element, ids) {
    if (!(element instanceof HTMLElement)) return ids;
    if (element.hasAttribute("id")) ids.push(element.id);
    if (!element.parentNode) return ids;
