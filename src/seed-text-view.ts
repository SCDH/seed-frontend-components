...
import {
    initTextView,
    setText as setTextViewText,
    scrolledTo,
    fetchAnnotationsPerSegment,
} from "./redux/textViewsSlice";
import {
    annotationSelected,
    annotationsPassedBy,
} from "./redux/annotationsSlice";
import {
    selectAnnotationsAtSegmentThunk,
    passByAnnotationsAtSegmentThunk,
} from "./redux/selectAnnotations";
...
export class SeedTextView extends windowMixin(storeConsumerMixin(LitElement)) {
        ...
        this.store?.dispatch(
            addAppListener({
                predicate: (
                    ...
                    return (
                        currentState.textViews.hasOwnProperty(this.id) &&
                        currentState.textViews[this.id].cssPerSegment !==
                            previousState.textViews[this.id].cssPerSegment
                    );
                },
                effect: (_action, listenerApi): void => {
                    ...
                    this.colorizeText(
                        listenerApi.getState().textViews[this.id].cssPerSegment,
                    );
                },
            }),
        );
            ...
            addAppListener({
                predicate: (
                    ...
                    return (
                        ...
                        currentState.texts[
                            currentState.textViews[this.id]?.textId ?? "unknown"
                        ] !==
                            previousState.texts[
                                currentState.textViews[this.id]?.textId ??
                                    "unknown"
                            ]
                    );
                },
        ...
        this.store?.dispatch(
            addAppListener({
                ...
                effect: (_action, listenerApi): void => {
                    ...
                    this.annotationSelected =
                        listenerApi.getState().annotations.annotationSelected ??
                        "unknown";
        ...
        this.store?.dispatch(
            addAppListener({
                ...
                effect: (_action, listenerApi): void => {
                    ...
                    const annotIds: Array<string> =
                        listenerApi.getState().annotations.annotationsTransient;
    ...
    connectedCallback() {
        ...
        this.store?.dispatch(
            setTextViewText({ viewId: this.id, text: this.id }),
        );
    }
    ...
    protected firstUpdated(_changedProperties: PropertyValues<this>): void {
        ...
        if (this.annotationsPerSegmentUrl) {
            this.store?.dispatch(
                fetchAnnotationsPerSegment({
                    viewId_: this.id,
                    url: this.annotationsPerSegmentUrl,
                }),
            );
        }
    }
    ...
    protected updated(_changedProperties: PropertyValues<this>): void {
        ...
        this.colorizeText(
            this.store?.getState()?.textViews?.[this.id]?.cssPerSegment,
        );
        ...
        if (this.iframe) {
            ...
            this.store?.dispatch(
                addAppListener({
                    ...
                    effect: setScrollTarget(
                        this.id,
                        this.scrollTextToThunk(
                            this.iframe,
                            this.getIFrameTarget(),
                        ),
                    ),
    ...
    protected handleMessage = (e: MessageEvent) => {
        if (this.iframe && e.source === this.iframe.contentWindow) {
            ...
            switch (e.data?.event) {
                case "loaded":
                    log.debug(
                        "document loaded, posting messages in queue: ",
                ...
                case "meta":
                    ...
                    const txt: TextState = {
                        location:
                            this.iframe.contentDocument?.location?.toString() ??
                            null,
                    ...
                    this.store?.dispatch(
                        setText({ textId: this.id, text: txt }),
                    );
                    break;
                case "scrolled":
                    this.store?.dispatch(
                        scrolledTo({ viewId: this.id, position: e.data.top }),
                    );
                    this.store?.dispatch(
                        scrolledTextViewThunk(scrolled, this.id, [e.data.top]),
                    );
                    break;
    ...
    colorizeText(
        ...
        if (this.iframe && cssPerSegment) {
            log.debug(
                `colorizing text in widget ${this.id}: ${Object.keys(cssPerSegment).length} segments`,
            );
            const msg = {
                event: "colorize",
     ...
     */
    protected colorizeAnnotation(
        state: SeedState,
        annotId: string,
        css: CSSDefinition,
    ): void {
        if (
            state.textViews[this.id].segmentsPerAnnotation.hasOwnProperty(
                annotId,
            )
        ) {
            log.debug(
                "colorizing selected annotation in " + this.id + ": " + annotId,
            );
            const segments: Array<string> =
                state.textViews[this.id].segmentsPerAnnotation[annotId];
