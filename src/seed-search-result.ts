...
export class SeedSearchResult extends searchResultMixin(LitElement) {
	...
	this.store?.dispatch(addAppListener({
		...
		const queryId: string = searchApi.endpoints.fields.name + "(\"" + this.collection + "\")";
		const flds: Array<string> = listenerApi.getState().searchApi?.queries?.[queryId]?.data as Array<string> ?? [];
		const regex: RegExp = new RegExp(this.pattern);
		const fl: Array<string> = flds.filter(f => f.match(regex));
    ...
    override updateEffect(endpoint: string, listenerApi: SeedListenerApi): void {
	const queryId: string = endpoint + "(\"" + solrSearchQuery(listenerApi.getState().searchQuery).replaceAll("\"", "\\\"") + "\")";
	log.debug("updating search result", queryId, listenerApi.getState().searchApi.queries.hasOwnProperty(queryId));
	...
	return html`<div>
		...
		${this.documents.map(d => this.renderDocument(d, this.pattern))}
	    </div>
            </div>`;
    }
