...
export class SeedSearchFilters extends storeConsumerMixin(LitElement) {
	...
	this.store?.dispatch(addAppListener({
		...
		this.filters.push(JSON.parse(JSON.stringify(action.payload))); // deep copy
		this.count++;
	    },
	}));
	// filter removed
	this.store?.dispatch(addAppListener({
		...
		this.filters = this.filters.filter(f => !(f.field == action.payload.field && f.term == action.payload.term));
		this.count--;
	    }
	}));
    }
	...
	return html`<seed-search-filter field="${filter.field}" term="${filter.term}"}></seed-search-filter>`;
    }
