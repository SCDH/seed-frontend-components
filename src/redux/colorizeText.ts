...
export const preferredColorPredicate: string =
    "https://intertextuality.org/annotation#preferredCssColor";
export const colorPriorityPredicate: string =
    "https://intertextuality.org/annotation#colorPriority";
...
export const setCssAnnotationsThunk = () => {
    return (dispatch: any, getState: any) => {
        ...
        const annots: { [key: AnnotationId]: Annotation } =
                    ...
                    for (const obj of annotation.predications[pred]) {
                        ...
                        if (obj.type === "resource") {
                            ...
                            if (ontology.hasOwnProperty(clasUri)) {
                                ...
                                if (
                                    tag.hasOwnProperty(preferredColorPredicate)
                                ) {
                                    var priority: number = 0;
                                    if (
                                        tag.hasOwnProperty(
                                            colorPriorityPredicate,
                                        )
                                    ) {
                                        priority =
                                            +tag[colorPriorityPredicate][0]
                                                .value;
                                    }
                                    css[annotId][priority] = {};
                                    css[annotId][priority][visualCategory] =
                                        tag[preferredColorPredicate][0].value;
                                }
                            }
                        }
                    }
...
export const setCssForAllSegmentsThunk = (textWidget: string) => {
    return (dispatch: any, getState: any) => {
        ...
        const {
            textViews,
            annotations,
        }: { textViews: TextViewsSlice; annotations: AnnotationsSlice } =
            getState();
        const annotsPerSegment: AnnotationsPerSegment | undefined =
            textViews[textWidget].annotationsPerSegment;
        const cssPerAnnotation: {
            [key: string]: { [priority: number]: CSSDefinition };
        } = annotations.cssPerAnnotation;
        if (
            annotsPerSegment !== undefined &&
            Object.keys(cssPerAnnotation).length > 0
        ) {
            ...
            for (const segment in annotsPerSegment) {
                if (segment !== undefined && segment !== "") {
                    ...
                    for (const annotId of annotsPerSegment[segment]) {
                        log.debug(
                            "annotation on segment " + segment + ": " + annotId,
                        );
                        // get the annotation by ID
                        const annotationCss: {
                            [priority: number]: CSSDefinition;
                        } = cssPerAnnotation[annotId];
                        ...
                        for (const priority in annotationCss) {
                            ...
                            if (priorityTyped > highestPriority) {
                                ...
                                for (const prop in annotationCss[priority]) {
                                    log.debug(
                                        "overwriting CSS property with higher priority",
                                        prop,
                                    );
                                ...
                                for (const prop in annotationCss[priority]) {
                                    if (!css.hasOwnProperty(prop)) {
                                        log.debug(
                                            "appending unprecedented CSS class with lower priority",
                                            prop,
                                        );
                                        css[prop] =
                                            annotationCss[priority][prop];
                                    }
                                }
                            }
                        }
                    ...
                    log.debug(
                        "setting CSS properties on segment",
                        segment,
                        css,
                    );
                    ...
                    log.warn(
                        "bad segment ID %s in text widget %s",
                        segment,
                        textWidget,
                    );
                }
...
export const subscribeAnnotationsCssUpdater = (store: SeedStore) => {
    store.subscribe(() => {
        ...
        if (
            state.ontology !== previousOntology ||
    ...
    store.subscribe(() => {
        ...
        if (state.textViews !== previousTextViews) {
            for (const viewId in state.textViews) {
                if (
                    !previousTextViews.hasOwnProperty(viewId) ||
                    (state.textViews[viewId] !== previousTextViews[viewId] &&
                        state.textViews[viewId].annotationsPerSegment !==
                            previousTextViews[viewId].annotationsPerSegment)
                ) {
 ...
 */
let previousCssPerAnnotation: {
    [key: AnnotationId]: { [priority: number]: CSSDefinition };
} = {};
