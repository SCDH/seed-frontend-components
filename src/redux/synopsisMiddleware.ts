...
export type PositionChangedEffect = (
    _action: UnknownAction,
    _listenerApi: ListenerEffectAPI<SeedState, SeedDispatch, unknown>,
) => void | Promise<void>;
...
export const setScrollTarget = (
    textViewId: string,
    scroll: (scrollTo: string) => void,
): PositionChangedEffect => {
    return (
        _action: UnknownAction,
        listenerApi: ListenerEffectAPI<SeedState, SeedDispatch, unknown>,
    ): void => {
        ...
        const targetTextId: string | undefined =
            textViewsSlice?.[textViewId]?.textId;
        if (targetTextId === undefined) return;
        // get scroll position of the text to be synced with
        const sourceTextId: string =
            synopsisSlice?.currentPosition?.textId ?? "unknown";
        const sourceSegmentIds: Array<string> =
            synopsisSlice?.currentPosition?.segmentIds ?? [];
        // possible by regexAlignment?
        const patterns =
            synopsisSlice?.regexAlignment?.[sourceTextId]?.[targetTextId] ?? [];
        for (const pattern of patterns) {
            const re = new RegExp(pattern.matchPattern);
            for (const sourceSegmentId of sourceSegmentIds) {
                if (re.test(sourceSegmentId)) {
                    scroll(
                        sourceSegmentId.replace(re, pattern.replacementPattern),
                    );
                    return; // done
                }
            }
