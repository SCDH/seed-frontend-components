...
export const addTextViewListeners = (startListening: startAppListeningType) => {
    ...
    startListening({
        ...
        effect: (_action, listenerApi): void => {
            ...
            const textViewsSlice: TextViewsSlice =
                listenerApi.getState().textViews;
            Object.keys(textViewsSlice).forEach((textViewId: string) => {
                const annotsPerSegment: AnnotationsPerSegment =
                    textViewsSlice[textViewId].annotationsPerSegment;
                // start with empty object
                const segmentsPerAnnot: SegmentsPerAnnotation = {};
                Object.keys(annotsPerSegment)
                    .filter((segmentId) => segmentId.length > 0)
                    .forEach((segmentId: string) => {
                        const annots: Array<string> =
                            annotsPerSegment[segmentId];
                        annots.forEach((annotId: string) => {
                            if (segmentsPerAnnot.hasOwnProperty(annotId)) {
                                segmentsPerAnnot[annotId].push(segmentId);
                            } else {
                                segmentsPerAnnot[annotId] = new Array<string>(
                                    1,
                                ).fill(segmentId);
                            }
                        });
                ...
                listenerApi.dispatch(
                    setSegmentsPerAnnotation({
                        viewId: textViewId,
                        segmentsPerAnnot: segmentsPerAnnot,
                    }),
                );
            });
        },
