 ...
 */
export const widgetSizeProvider = <T extends Constructor<LitElement>>(
    superClass: T,
) => {
    class WidgetSizeProvider extends superClass {
        ...
        recalculateChildDimensions() {
            ...
            const lineHeightStr: string =
                window
                    ...
                    ?.match(/\d+/)?.[0] ?? this.defaultLineHeightPx;
            const lineHeight: number = Number(lineHeightStr);
            if (this.orientation === "horizontal") {
                this.widgetHeight = this.offsetHeight;
                this.widgetWidth =
                    (this.offsetWidth -
                        this.childrenCountMinimized *
                            this.widgetWidthMinimized - // minus minimized
                ...
                this.widgetHeight =
                    (this.offsetHeight -
                        this.childrenCountMinimized *
                            this.widgetHeightMinimized - // minux minimized
        ...
        handleChildEvent() {
            return (e: Event) => {
                const { oldWindowState, newWindowState, initialize } = (
                    e as CustomEvent<{
                        oldWindowState: WindowState | undefined;
                ...
                if (initialize === true) {
                    ...
                    if (newWindowState === WindowState.Container) {
                        ...
                        log.error(
                            "inconsistent window state: initializing with disposed state",
                        );
                    } else {
                    ...
                    if (oldWindowState === WindowState.Container) {
                        ...
                        log.error(
                            "unknown state of disposed window: " +
                                newWindowState,
                        );
                    }
                    this.childrenCount -= 1;
                } else if (
                    oldWindowState === WindowState.Minimized &&
                    newWindowState === WindowState.Container
                ) {
                    this.childrenCountMinimized -= 1;
                    this.childrenCountContainer += 1;
                } else if (
                    oldWindowState === WindowState.Container &&
