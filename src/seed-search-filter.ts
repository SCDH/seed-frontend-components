...
export class SeedSearchFilter extends storeConsumerMixin(LitElement) {
	...
	if (Object.keys(this.store?.getState()?.searchQuery?._fq_faceted ?? {}).length > 0) {
	    this.store?.dispatch(searchApi.endpoints.filter.initiate(this.store?.getState()?.searchQuery));
	}
    }
