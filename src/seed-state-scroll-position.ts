        ...
        this.store?.dispatch(
            addListener({
                predicate: (
                    ...
                    let currState: { textViews: TextViewsSlice } =
                        currentState as SeedState;
                    return (
                        currState.textViews.hasOwnProperty(
                            this.textView?.id ?? "_",
                        ) &&
                        currState.textViews[this.textView?.id ?? "_"]
                ...
                effect: (_action, listenerApi): void => {
                    ...
                    this.position =
                        state.textViews[this.textView?.id ?? "_"]
        ...
        return html`<span class="window-status-item">
            <span class="scroll-position">
                ...
                <button
                    ...
                    @click="${this.handleScrollToInput}"
                    title="Go to manually entered position!"
                >
                    &#x23ce;
                </button>
            </span>
