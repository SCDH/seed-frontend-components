 ...
 */
export const searchResultMixin = <T extends Constructor<LitElement>> (superClass: T) => {
	...
	override subscribeStore() {
		...
		effect: async (_action, listenerApi) => this.updateEffect(searchApi.endpoints.documents.name, listenerApi),
	    }));
	    // get result when a filter was applied and the query was initiated
	    this.store?.dispatch(addAppListener({
		matcher: searchApi.endpoints.filter.matchFulfilled,
		effect: async (_action, listenerApi) => this.updateEffect(searchApi.endpoints.filter.name, listenerApi),
	    }));
